      ...
      if (categoria === 'todas') {
        ...
        Object.values(DATOS.empresas).forEach(empresasCategoria => {
          if (Array.isArray(empresasCategoria)) {
            empresasCategoria.forEach(empresa => contenedor.appendChild(this.crearTarjetaEmpresa(empresa)));
          }
        });
      } else {
        ...
        if (empresas.length === 0) {
          ...
          empresas.forEach(empresa => contenedor.appendChild(this.crearTarjetaEmpresa(empresa)));
        }
      }
...
export const controladorCompeticiones = {
  ...
  filtrarCompeticiones(categoria = 'todas') {
    ...
    const botonActivo = utilidades.obtenerElemento(`.btn-categoria[onclick*="filtrarCompeticiones('${categoria}')"]`);
    if (botonActivo) botonActivo.classList.add('active');
      ...
      if (categoria === 'todas') {
        ...
        Object.values(DATOS.competiciones).forEach(categoriasComp => {
          if (Array.isArray(categoriasComp)) {
            competicionesAMostrar = [...competicionesAMostrar, ...categoriasComp];
          }
        });
      } else {
        // Mostrar por categoría específica
...
async function inicializarApp() {
  ...
  try {
    ...
    if (!window.filtrarCompeticiones) window.filtrarCompeticiones = categoria => controladorCompeticiones.filtrarCompeticiones(categoria);
